...
valid_path = ""
test_path = ""
train_path = train_path + 'train1.txt'
valid_path = valid_path + 'valid1.txt'
test_path = test_path + 'test1.txt'
train_data = Model_attention.Dataset(train_path, src_field=SRC, trg_field=TRG)
valid_data = Model_attention.Dataset(valid_path, src_field=SRC, trg_field=TRG)
test_data = Model_attention.Dataset(test_path, src_field=SRC, trg_field=TRG)
...
attn = Attention.Attention(Paras.ENC_HID_DIM, Paras.DEC_HID_DIM)
enc = Model_attention.attn_Encoder(INPUT_DIM, Paras.ENC_EMB_DIM, Paras.ENC_HID_DIM, Paras.DEC_HID_DIM, Paras.N_LAYERS, Paras.ENC_DROPOUT)
dec = Model_attention.attn_Decoder(OUTPUT_DIM, Paras.DEC_EMB_DIM, Paras.ENC_HID_DIM, Paras.DEC_HID_DIM, Paras.N_LAYERS, Paras.DEC_DROPOUT, attn)

model = Seq2Seq_attn.attn_Seq2Seq(enc, dec, device).to(device)

### init_weights

model.apply(Model_attention.init_weights)

### count_parameters

print(f'The model has {Model_attention.count_parameters(model):,} trainable parameters')
...
for epoch in range(N_EPOCHS):
    ...
    train_loss = Train.train(model, train_iterator, optimizer, criterion, CLIP)          ##### error occured (2nd loop)
    # print("train_loss: ")
    # print(train_loss)
